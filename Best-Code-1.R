# ---- Auto-detect melt mode per sample with unified robust Tonst (amp+slope + guardrails + pattern overrides + final fallback) ----
...
# Global guardrails to rein-in late Tonst (default for all wells)
early_cap_frac  <- 0.25   # cap Tonst at ≤ 25% of baseline→extremum change
cap_delta_deg   <- 8      # cap Tonst at ≤ (T_extremum − 6 °C)

# --- Per-sample Tonst guardrail overrides (PATTERN-based for robustness) ---
# These regex patterns will match your two wells even if labels vary slightly.
# Escape a literal string so it can be used safely inside a regex
regex_escape <- function(x) {
  gsub("([][{}()+*^$|\\.?*\\\\])", "\\\\\\1", x)
}

caps_override_patterns <- list(
  "A0\\.1.*5" = list(early_cap_frac = 0.30, cap_delta_deg = 8),  # matches "A0.1...5"
  "A0\\.5.*9" = list(early_cap_frac = 0.30, cap_delta_deg = 8)   # matches "A0.5...9"
)
...
# After df_num is built:
sample_names <- setdiff(names(df_num), "Temperature")
targets <- sample_names[c(4, 8)]  # 5th and 9th sample columns

caps_override_patterns <- setNames(
  replicate(length(targets), list(list(early_cap_frac = 0.30, cap_delta_deg = 8)), simplify = FALSE),
  paste0("^", sapply(targets, regex_escape), "$")
)
...
tonst_core <- function(T, Y, dir = +1,
  ...
  # Align slope flag to points grid and enforce persistence there too
  ...
  if (is.finite(j)) {
    ...
    # fractional fallback
    frac_target <- if (dir == +1) base_med + frac_fb * total_change else base_med - frac_fb * total_change
    cross_frac <- if (dir == +1) (y_pre >= frac_target) else (y_pre <= frac_target)
    j <- which(cross_frac)[1]
    if (is.finite(j)) {
      if (j == 1) { Tonst_cand <- T[1]; Y_on <- y_pre[1]; method <- sprintf("frac_only(%.0f%%)-edge", 100*frac_fb) }
      else { i <- j - 1; Tonst_cand <- lin_interp_x(T[i], y_pre[i], T[i+1], y_pre[i+1], frac_target); Y_on <- frac_target; method <- sprintf("frac_only(%.0f%%)", 100*frac_fb) }
    } else {
      # ---- FINAL FALLBACK: use early cap fraction position if set ----
  ...
  # Early-cap by fraction of total change (pre-extremum)
  if (is.finite(early_cap_frac) && early_cap_frac > 0 && early_cap_frac < 1) {
  ...
  # Cap by minimum gap from extremum (e.g., Tonst ≤ T_ext - 6°C)
...
results <- long %>%
  ...
  group_modify(~{
    ...
    # Per-sample caps: take the group key from .y (not .x)
    smpl <- .y$Sample[[1]]
...
# Save table
readr::write_csv(results, "Tm_Tonst_autodetect_guardrails_patternOverrides.csv")
print(results)
...
p_all <- ggplot(long, aes(x = Temperature, y = Signal, group = Sample)) +
  ...
  labs(title = "Auto-detected melt behavior with unified robust Tonst (dual criteria + guardrails + pattern overrides)",
       x = "Temperature (°C)", y = "Signal",
       caption = sprintf("LOESS=%.2f, runmed=%s(k=%d). Amp: k=%.1f floor=%.0f%%; Slope: k=%.1f floor=%.0f%%; consec=%d; early_cap=%.0f%%; Δcap=%g°C",
...
ggsave("Melt_curves_all_autodetect_guardrails_patternOverrides.png", p_all, width = plot_w_all, height = plot_h_all, dpi = 300)
...
p_facet <- ggplot(long, aes(x = Temperature, y = Signal)) +
  ...
  labs(title = "Per-sample curves with unified robust Tonst (overrides + final fallback)",
       x = "Temperature (°C)", y = "Signal") +
  theme_minimal(base_size = 11)
ggsave("Melt_curves_facet_autodetect_guardrails_patternOverrides.png", p_facet, width = plot_w_fac, height = plot_h_fac, dpi = 300)
